...
public class CoApplicantSignatureConfirmationActivity extends BaseActivity implements View.OnClickListener {
...
//                startActivity(new Intent(PostApprovalSignatureConfirmationActivity.this,PostApprovalSignatureConfirmationActivity.class));
    ...
    private void submitDeclaration() {
        ...
        showProgressDialog(CoApplicantSignatureConfirmationActivity.this);
        try {
            ...
            if (GlobalValue.signatureUriPath != null) {
                ...
                fileToUpload = MultipartBody.Part.createFormData("document[co_applicant_signature]", file.getName(), mFile);
            }
                ...
                public void onResponse(Call<LoanTakerPostApprovalDocumentResponseModel> call, Response<LoanTakerPostApprovalDocumentResponseModel> response) {
                    ...
                    if (response.isSuccessful()) {
                        if (response.body().getSuccess()) {
                            ...
                            params.putString("status","post_approval_co_applicant_signature_created");
                            mFirebaseAnalytics.logEvent("post_approval_co_applicant_signature", params);

                            updateStatusinParentPage();
//                            goToInsuranceForm();
                            Toast.makeText(CoApplicantSignatureConfirmationActivity.this, "" + response.body().getNotice(), Toast.LENGTH_SHORT).show();
                            ...
                            finishButton.setEnabled(true);
                            finishButton.setBackground(getResources().getDrawable(R.drawable.primary_button_bg));
                            NetworkUtils.handleErrorsCasesForAPICalls(CoApplicantSignatureConfirmationActivity.this, response.code(), response.body().getErrors());
                        }
                    } else {
                        Log.d("Drools onResponse3", "" + response.body().getErrors());
                        finishButton.setEnabled(true);
                        finishButton.setBackground(getResources().getDrawable(R.drawable.primary_button_bg));
                        NetworkUtils.handleErrorsForAPICalls(CoApplicantSignatureConfirmationActivity.this, null, null);
                    }
    ...
    private void updateStatusinParentPage() {
        PostDocumentsDeclarationActivity.getInstance().postSignatures();
    }
    ...
    private  void goToInsuranceForm(){
        ...
        Intent intent = new Intent(CoApplicantSignatureConfirmationActivity.this,IndividualMemberDetailActivity.class);
