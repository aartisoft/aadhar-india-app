...
public class ApplicantSignatureConfirmationActivity extends BaseActivity implements View.OnClickListener {
...
//                startActivity(new Intent(PostApprovalSignatureConfirmationActivity.this,PostApprovalSignatureConfirmationActivity.class));
    ...
    private void submitDeclaration() {
        ...
        showProgressDialog(ApplicantSignatureConfirmationActivity.this);
        try {
            ...
            if (GlobalValue.signatureUriPath != null) {
                ...
                fileToUpload = MultipartBody.Part.createFormData("document[applicant_signature]", file.getName(), mFile);
            }
                ...
                public void onResponse(Call<LoanTakerPostApprovalDocumentResponseModel> call, Response<LoanTakerPostApprovalDocumentResponseModel> response) {
                    ...
                    if (response.isSuccessful()) {
                        if (response.body().getSuccess()) {
                            ...
                            params.putString("status","post_approval_applicant_signature_created");
                            mFirebaseAnalytics.logEvent("post_approval_applicant_signature", params);

                            updateStatusinParentPage();
//                            goToInsuranceForm();
                            Toast.makeText(ApplicantSignatureConfirmationActivity.this, "" + response.body().getNotice(), Toast.LENGTH_SHORT).show();
                            ...
                            finishButton.setEnabled(true);
                            finishButton.setBackground(getResources().getDrawable(R.drawable.primary_button_bg));
                            NetworkUtils.handleErrorsCasesForAPICalls(ApplicantSignatureConfirmationActivity.this, response.code(), response.body().getErrors());
                        }
                    } else {
                        Log.d("Drools onResponse3", "" + response.body().getErrors());
                        finishButton.setEnabled(true);
                        finishButton.setBackground(getResources().getDrawable(R.drawable.primary_button_bg));
                        NetworkUtils.handleErrorsForAPICalls(ApplicantSignatureConfirmationActivity.this, null, null);
                    }
    ...
    private  void goToInsuranceForm(){
        ...
        Intent intent = new Intent(ApplicantSignatureConfirmationActivity.this,IndividualMemberDetailActivity.class);
